...
> 演示站从本仓 `flows/` 副本加载全部共享流程 JSON（15 个；唯一编辑源在 `jeeflow-java` 仓，`flows-resolver` 在维护者机器上执行时精确镜像进本仓，单语言用户下载即用）。对接 jeeflow-ui（:5173）时右上角切到 `🟢 Node :8082`；接口规范见[统一门面接口文档](../../spec/06-facade)。
